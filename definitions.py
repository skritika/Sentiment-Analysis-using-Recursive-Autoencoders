...
class tree:
	...
	def forward(self, freq, W1, W2, W3, W4, Wcat, b1, b2, b3, bcat, alpha, beta, sentence_label):
		...
		for j in range(0,sl-1):
			...
			y1_norm, y2_norm = y1, y2 #only for normalized tanh
	...
	def cost(self, w, W1, W2, W3, W4, Wcat, b1, b2, b3, bcat, alpha, beta, sentence_label):
		...
		for j in range(0,sl-1):
			...
			nF[:,j:j+1] = f(np.dot(W1,c1)+np.dot(W2,c2)+b1)
			y1, y2 = f(np.dot(W3,nF[:,j:j+1])+b2), f(np.dot(W4,nF[:,j:j+1])+b3)
			y1c1, y2c2 = alpha*(y1-c1), alpha*(y2-c2)
			nodeScoresR[sl+j] = sum(y1c1*(y1-c1)+y2c2*(y2-c2))*0.5
		out = fcat(np.dot(Wcat,words)+np.tile(bcat,sl))
		diff = np.tile(sentence_label,sl)-out
		lbl_sm = (1-alpha)*diff
		score = 0.5*lbl_sm*diff
		nodeScores[0:sl] = score.T
		for i in range(sl,2*sl-1):
			sm = fcat(np.dot(Wcat,nF[:,i]) + bcat)
			lbl_sm = beta*(1-alpha)*(sentence_label-sm)
			nodeScores[i] = 0.5*(np.dot(lbl_sm.T,(sentence_label-sm)))
	...
	def checkgradient(self, freq, eps, W1, W2, W3, W4, Wcat, b1, b2, b3, bcat, alpha, beta, sl):
		...
		j2 = self.cost(w,W1b,W2,W3,W4,Wcat,b1,b2,b3,bcat,alpha,beta,sl)
		return (j1-j2)/(2*eps)
